...
import { EditAnswerDto, EditQuestionDto, QuestionTypeEnum, SkillDto, SkillLevelEnum } from 'src/app/shared/model';
...
@Component({
  selector: 'app-multiple-answers-question-edit',
  templateUrl: './multiple-answers-question-edit.component.html',
  styleUrls: ['./multiple-answers-question-edit.component.scss']
})
export class MultipleAnswersQuestionEditComponent implements OnInit {
  ...
  public changeValueOfAnswer(event: any, answerId: string): void {
    this.question.answers.forEach(answer => {
      if (answer.id == answerId) answer.valid = event.checked;
    });
  }
}